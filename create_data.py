...
# Get the weighted batting statistics for each hitter
def calculate_batting_stats(batting_stats, player, year):
    ...
    # Convert 2020 stats to 162-game scale
    if len(index_2020) > 0:
    ...
    # If there is only one season of data, the average will just be the stats for that year
...
# Get the weighted pitching statistics for each pitcher
...
# Cleaning the previous free agents dataset
...
# Replace accented characters in previous free agent names
...
# The position will only be the player's main position
...
# Convert age to numeric integers
...
# Convert contracts to present-day values
...
# Replace accented characters in batting stats names
...
# Replace accented characters in pitching stats names
...
# Convert previous free agents dataframe to sql and store is in the db
# Perform SQL queries to fetch the data
...
# Apply function to get all players' hitting statistics
...
# Adjust Ohtani's contract as a hitter
...
# Apply function to get all players' pitching statistics
...
# Adjust Ohtani's contract as a starter
...
# Replace all the accents in current free agents
...
# Convert dataframe to database table in SQL
...
# Get all batting statistics for upcoming free agents
...
# Get all pitching statistics for upcoming free agents
...
# Randomize rows for each dataframe and split data into batters, starters, and relievers
...
# Training and testing features for hitters------------------------------------
...
# Years model conditions (hitters)
conditions = [
...
# Apply XGBoost model to predict years
...
# Apply XGBoost model to predict AAV
...
# Training and testing features for starting pitchers------------------------------------
...
# Years model conditions (starters)
conditions = [
...
# Training and testing features for relief pitchers------------------------------------
...
# Years model conditions (relievers)
conditions = [
...
# AAV model conditions (relievers)
conditions = [
    (X_train_relief['AVG_FIP (2 Yrs)'] <= 3.0),
    (X_train_relief['AVG_FIP (2 Yrs)'] <= 3.5),
]
...
# Update players projected contracts for hitters
...
# Update players projected contracts for starters
...
# Update players projected contracts for relievers
